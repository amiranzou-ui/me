...
The site is a hand-authored artifact — a cinematic "Matrix vs Human" split identity, a hidden node-graph "Mind" page, custom canvas ambient effects — that is genuinely this person's permanent digital home, not a disposable portfolio project. Its problem isn't design, it's maintainability: every new project, photo, or song requires editing large JS files by hand. That breaks the creative workflow and doesn't scale to the 100+ projects / 500+ photos / 20+ categories this site should hold over the next 5-10 years.
...
- **The hidden Mind page's feel** — desktop-only gating, the node-graph/journal authoring experience, the soft "curiosity" pacing (min-visits, hour-range gating) — preserved as a UX, only its data backend changes.
...
| Mind subsystem: 100% `localStorage`, zero backend | Supabase-backed authored content (nodes/connections/journal), with visitor pacing state (`mind_v`, `mind_seen`, interaction timestamps) staying in `localStorage` | Today the owner's authored graph never reaches visitors on other devices — this fixes that gap, while keeping each visitor's own curiosity-pacing state local since there's no visitor account system |
...
Visitor's browser
   │
   ├─ Mind page: fetches authored graph/journal from Supabase,
   │  but keeps its OWN visit-count/interaction pacing state in
...
- `matrix_nodes` — the CV page's own "system view" graph. Deliberately named apart from Mind's node tables; these are two unrelated graph concepts that happen to share a visual metaphor.

**Mind subsystem**
- `mind_nodes` (node_key, type, content, fragment, visibility_mode, conditions jsonb, emotion_weight, position, delay, `parent_node_key` for nested thought-children, connections cache).
- `mind_connections` (from/to node_key, type: logical|contradiction|echo|hidden).
- `mind_journal_entries` — a fully separate store from the node graph, as it is today.
...
| 4 — Mind system port | `mind_nodes`/`mind_connections`/`mind_journal_entries`; visitor rendering ported as-is (pacing state stays localStorage); `editor.js`/`mind-author.js` persistence swapped to Supabase | Owner authors once, visible from any device — fixes the current cross-device gap |
...
| 6 — Mobile pass + perf/a11y audit | Dedicated (not compressed-desktop) mobile interactions; lighter Framer variants replacing heavy Canvas effects on touch; Lighthouse/axe pass | Mobile feels distinct and light; audit clean |
...
- **Mind's visitor pacing state stays client-side** — it's the visitor's own experience of curiosity/discovery, not authored content, and there's no visitor account system to attach it to server-side.
