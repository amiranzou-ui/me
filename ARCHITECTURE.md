...
- **The landing page's `.atm-human`/`.atm-matrix` photo layers were tried and reverted** — briefly ported from `atmosphere.css` during Phase 7 (the parallax JS in `world-effects.ts` already handled them gracefully), but removed after seeing them live — felt like visual clutter rather than atmosphere. `world-effects.ts`'s atmosphere-parallax code is left in place (still a harmless no-op with no `.atm-*` elements in the DOM) in case this is revisited differently later.
...
| 7 — Cutover ✅ | Full content diff old vs. new (surfaced and closed the intro/atmosphere gaps above; no production migration re-run needed — Phases 1-3 already wrote real content into the one live Supabase project); `ameerstudio.site` added to Vercel and DNS repointed there before merging `main` (so the switch was invisible mid-flight — both hosts served identical old content during propagation); `v2` → `main` merged and pushed once DNS was confirmed; `legacy/` (old HTML/CSS/JS) deleted from the working tree post-cutover | `ameerstudio.site` serves the new site — confirmed live: intro sequence, friend gate, archive hall, Matrix CV data, and Studio login all verified working on the real domain |
